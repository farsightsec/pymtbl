...
class MergerTestCase(MtblTestCase):
    ...
    def do_merge(self, merge_func):
        ...
        merged_filename = os.path.join(os.path.dirname(__file__), "merged.mtbl")
        writer = mtbl.writer(merged_filename, compression=mtbl.COMPRESSION_NONE)
    ...
    def test_merge_and_iteritems_no_merge(self):
        ...
        for filename, tuples in [
            ("left.mtbl", [("key1", "val1"), ("key2", "val2"), ("key3", "val3")]),
            ("right.mtbl", [("key17", "val17"), ("key23", "val23"), ("key4", "val4")]),
        ]:
    ...
    def test_merge_and_iteritems_with_merge(self):
        ...
        for filename, tuples in [
            ("left.mtbl", [("key1", "val1"), ("key2", "val2"), ("key3", "val3")]),
            ("right.mtbl", [("key1", "val12"), ("key2", "val22"), ("key3", "val32")]),
        ]:
            m = self.write_mtbl(filename, tuples)
            self.mtbls_to_merge.append(m)
        result = self.do_merge(merge_func_str)
    ...
    def test_merge_with_ints(self):
        ...
        for filename, tuples in [
            (
                "left.mtbl",
                [
                    ("key1", mtbl.varint_encode(1)),
                    ("key2", mtbl.varint_encode(42)),
                    ("key3", mtbl.varint_encode(12345)),
                ],
            ),
            (
                "right.mtbl",
                [
                    ("key1", mtbl.varint_encode(2)),
                    ("key2", mtbl.varint_encode(1)),
                    ("key3", mtbl.varint_encode(128)),
                ],
            ),
        ]:
            m = self.write_mtbl(filename, tuples)
    ...
    def test_merge_iteritems_as_bytes(self):
        ...
        for filename, tuples in [
            ("left.mtbl", [("key1", "val1"), ("key2", "val2"), ("key3", "val3")]),
            ("right.mtbl", [("key1", "val17"), ("key2", "val23"), ("key4", "val4")]),
        ]:
            m = self.write_mtbl(filename, tuples)
            self.mtbls_to_merge.append(m)
